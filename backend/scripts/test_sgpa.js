...
async function testAggregation() {
    try {
        ...
        // Get a sample mark
        const sampleMark = await Mark.model.findOne();
        if (!sampleMark) {
            console.log('No marks found in "marks" collection.');
            return;
        }
        const studentId = sampleMark.student_id;
        ...
        // Check join success
        const joinCheck = await Mark.model.aggregate([
            { $limit: 1 },
            {
                $lookup: {
                    from: 'department_course',
        ...
        console.log('Join check (1 record):', JSON.stringify(joinCheck, null, 2));
