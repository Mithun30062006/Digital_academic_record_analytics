...
const DepartmentCourse = require('../backend/models/DepartmentCourse');

async function testAggregation() {
    try {
        const mongoUrl = process.env.MONGODB_URI || 'mongodb://localhost:27017/student_portal';
        console.log('Connecting to:', mongoUrl);
        await mongoose.connect(mongoUrl);
        console.log('Connected');

        // Get a sample student student_id
        const mark = await Mark.model.findOne();
        if (!mark) {
        ...
        const metrics = await Mark.model.aggregate([
            ...
            // Just show the grouped data for now
            { $limit: 5 }
        ]);

        console.log('Aggregation result (sample):', JSON.stringify(metrics, null, 2));
